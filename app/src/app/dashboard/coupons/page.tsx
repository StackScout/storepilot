...
export default function DashboardCouponsPage() {
  ...
  const { data: coupons, isLoading } = useQuery({
    ...
    queryFn: () => couponsService.listStoreCoupons(storeId),
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => couponsService.deleteStoreCoupon(id),
    onSuccess: () => {
    ...
    <div className="max-w-6xl space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Discount codes buyers can apply at checkout for this store.</p>
        </div>
        <CouponFormDialog scope="store" storeId={storeId} />
      </div>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <EmptyState
              ...
              description="Create a discount code to offer buyers a percentage or fixed amount off."
            />
          ) : (
                    ...
                    <tr key={coupon.id} className="border-b last:border-0">
                      ...
                      <td className="px-6 py-3">
                        <div className="flex items-center gap-1">
                          <CouponFormDialog coupon={coupon} scope="store" storeId={storeId} />
                          <Button
                            variant="ghost"
                            size="icon"
