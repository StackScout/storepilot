...
function Bubble({ message }: { message: MessageResponse }) {
  ...
  const isOwn = message.senderType === 'buyer';
  return (
    <View style={[styles.bubbleRow, isOwn && styles.bubbleRowRight]}>
      <View style={[styles.bubble, { backgroundColor: isOwn ? '#208AEF' : theme.backgroundElement }]}>
        <ThemedText style={isOwn ? styles.bubbleTextOwn : undefined}>{message.body}</ThemedText>
      </View>
...
export default function BuyerConversationThreadScreen() {
            ...
            ListHeaderComponent={
              conversationQuery.data ? (
                <ThemedText type="smallBold" style={styles.header}>
                  {conversationQuery.data.storeName}
                </ThemedText>
              ) : null
            }
          />
