    ...
    <View style={[styles.row, { backgroundColor: theme.backgroundElement }]}>
      <TouchableOpacity onPress={onPress} style={styles.rowMain}>
        ...
        <View style={styles.rowText}>
          ...
          <ThemedText type="small" themeColor={service.status === 'active' ? undefined : 'textSecondary'}>
            {service.status}
          </ThemedText>
        </View>
      </TouchableOpacity>
      <TouchableOpacity onPress={onDelete} style={styles.deleteButton}>
        <ThemedText style={{ color: '#D64545' }}>Delete</ThemedText>
      </TouchableOpacity>
    </View>
  );
}
...
export default function ServicesListScreen() {
  ...
  const deleteMutation = useMutation({
    ...
    onError: (e) => Alert.alert('Could not delete service', e instanceof ApiError ? e.message : 'Please try again.'),
  });

  const confirmDelete = (service: BookableServiceResponse) => {
    Alert.alert('Delete this service?', `"${service.name}" will no longer be bookable.`, [
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.background }} edges={['bottom']}>
      <FlatList
        ...
        renderItem={({ item }) => (
          <ServiceRow service={item} onPress={() => router.push(`/bookings/services/${item.id}`)} onDelete={() => confirmDelete(item)} />
        )}
        ListEmptyComponent={
          !servicesQuery.isLoading ? (
            <ThemedText themeColor="textSecondary" style={styles.empty}>
              No services yet — add your first bookable service below.
            </ThemedText>
          ) : null
        }
      />
      <TouchableOpacity style={styles.fab} onPress={() => router.push('/bookings/services/new')}>
...
const styles = StyleSheet.create({
  ...
  deleteButton: { paddingHorizontal: Spacing.two },
