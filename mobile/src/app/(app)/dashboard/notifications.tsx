...
import type { SellerNotificationResponse } from '@/api/types';
...
/** Mirrors _layout.tsx's routeForNotification — same type/id vocabulary as the push-tap deep link. */
function routeForNotification(notification: SellerNotificationResponse): Href | null {
...
function NotificationRow({ notification, onPress }: { notification: SellerNotificationResponse; onPress: () => void }) {
...
export default function NotificationsScreen() {
  ...
  const notificationsQuery = useQuery({
    queryKey: ['me', 'seller', 'notifications'],
  ...
  const onPressNotification = async (notification: SellerNotificationResponse) => {
    if (!notification.read) {
      queryClient.setQueryData(['me', 'seller', 'notifications'], (old: typeof notificationsQuery.data) =>
        old ? { ...old, content: old.content.map((n) => (n.id === notification.id ? { ...n, read: true } : n)) } : old,
      );
      markNotificationRead(notification.id).finally(() => {
        queryClient.invalidateQueries({ queryKey: ['me', 'seller', 'notifications', 'summary'] });
      });
    }
