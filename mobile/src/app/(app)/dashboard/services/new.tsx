...
import { createService, type BookableServiceFormInput } from '@/api/bookable-services';
import { getMyStore } from '@/api/stores';
import { ServiceForm, type ServiceFormValue } from '@/components/service-form';
import { ApiError } from '@/lib/api-client';
import { useTheme } from '@/hooks/use-theme';

function blankFor(category: string): BookableServiceFormInput {
  return { name: '', description: '', category, price: 0, durationMinutes: 30, bufferMinutes: 0, status: 'active' };
}
...
export default function NewServiceScreen() {
  ...
  const createMutation = useMutation({
    ...
    onError: (e) => Alert.alert('Could not create service', e instanceof ApiError ? e.message : 'Please try again.'),
  });
