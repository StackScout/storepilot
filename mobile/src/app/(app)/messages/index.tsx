...
import { listStoreConversations } from '@/api/messaging';
  ...
  return (
    <TouchableOpacity onPress={onPress} style={[styles.row, { backgroundColor: theme.backgroundElement }]}>
      <View style={styles.rowTop}>
        <ThemedText type="smallBold">{conversation.buyerName}</ThemedText>
...
export default function ConversationsListScreen() {
  ...
  const conversationsQuery = useQuery({
    queryKey: ['store', storeId, 'conversations'],
    queryFn: () => listStoreConversations(storeId!),
    enabled: !!storeId,
